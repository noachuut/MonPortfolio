            ...
            <div className="flex gap-5 items-start">
              ...
              <div className="flex flex-col gap-2 flex-1">
                <div className="flex justify-between items-center">
                  <h3 className="font-semibold text-primary group-hover:text-primary/90">{acc.name}</h3>
                  <Badge variant="secondary">{platformLabel(acc.platform)}</Badge>
                </div>
                <p className="text-sm text-muted-foreground leading-snug">{acc.description}</p>
              </div>
            </div>
          </Card>
        </a>
      ...
      <div className="container mx-auto px-6 py-16 max-w-6xl">
        ...
        <div className="text-center mb-16 fade-in">
          <h2 className="text-4xl md:text-5xl font-bold mb-6">
            Ma <span className="hero-gradient bg-clip-text text-transparent">Veille</span> technologique
          </h2>
        ...
        <section className="mb-16">
          <h2 className="text-3xl font-semibold mb-4 text-primary">daily.dev</h2>
          <Card className="bg-transparent card-gradient border border-border/60 rounded-2xl backdrop-blur-md hover:border-primary/40 transition-all duration-300 hover:shadow-lg hover:shadow-primary/10">
            <div className="grid md:grid-cols-3 gap-10 items-center p-8">
              <div className="md:col-span-1 flex justify-center">
                {profile.dailyDev?.devCardImage ? (
                  ...
                  <span className="text-sm text-gray-500">Image daily.dev manquante</span>
                )}
              </div>
                ...
                {profile.dailyDev?.profileLink && (
                  <div className="mt-5">
                    <Button asChild size="sm" variant="outline">
                      <a href={profile.dailyDev.profileLink} target="_blank" rel="noopener noreferrer">
        ...
        {/* RESEAUX SOCIAUX */}
        <section className="mb-16">
          <h2 className="text-3xl font-semibold mb-2 text-primary">Réseaux sociaux</h2>
          ...
          <Tabs defaultValue="all">
            ...
            <TabsContent value="all">{renderAccountsGrid(profile.socialAccounts || [])}</TabsContent>
            {Array.from(accountsByPlatform.entries()).map(([p, accounts]) => (
              <TabsContent key={p} value={p}>{renderAccountsGrid(accounts)}</TabsContent>
            ))}
          </Tabs>
        </section>
        ...
        {/* SUJET FAVORI */}
        <section>
          <h2 className="text-3xl font-semibold mb-6 text-primary">{profile.favoriteTopic.title}</h2>
          <Card className="p-8 bg-transparent card-gradient border border-border/60 rounded-2xl hover:border-primary/40 hover:shadow-primary/10 transition-all duration-300">
